...
Video streaming backend that processes uploaded videos into HLS format for streaming and serves both API and HLS static content.
...
├── server/
│   ├── server.go        # Server struct and wiring
...
├── @videos/             # Generated video data (raw + HLS)
└── cleanup.sh           # Helper script to clean generated video data
```
